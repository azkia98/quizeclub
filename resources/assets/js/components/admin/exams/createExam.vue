<template>
    <div>
        <div class="w3-panel w3-green w3-display-container w3-margin-top" v-if="message.show">
            <span @click="message.show=!message.show" class="w3-button w3-green w3-large w3-display-topright">&times;</span>
            <h3>Success!</h3>
            <p v-text="message.title"></p>
        </div>
        <div>
            <div v-text="examName" @dblclick="editExamName($event.target)" class="w3-center w3-xxlarge"
                 v-show="isshowExamTitle"></div>
            <input class="w3-border w3-border-0 w3-center mh-center" @blur="showExamTitle($event.target)"
                   v-if="!isshowExamTitle" v-model="examName" autofocus/>
        </div>
        <div v-for="(question,question_index) in questions" class="w3-panel w3-margin-top">
            ...
            <div class="w3-row-padding" style="margin-top:5px">
                <div class="w3-quarter" v-for="(answer,index) in question.answers">
                    <div class="w3-padding" v-text="answer"
                         :class="[ examObject.hasTrueAnswer(index,question.trueAnswer)?'w3-green':'w3-2017-greenery']"></div>
                </div>
            </div>
        </div>
        <div class="w3-panel">
            ...
            <div class="w3-row-padding w3-margin-top">
                <div class="w3-quarter" v-for="(answerInput,index) in answerInputNumber">
                    <input type="text" class="w3-input w3-border" v-model="answerInput.value"
                           @click.ctrl="setTrueAnswer(index)" @keyup.46="deleteInput(index)"
                           :class="{'w3-pale-green' : answerInput.isTrue}" placeholder="answer">
                </div>
                <button @click="addAnswerInput()" class="w3-rest fa fa-plus w3-button w3-round w3-green"
                        style="margin-top:0.5%"></button>
            </div>
            <div class="w3-panel">
            ...
            sendExamData() {
                axios.post(`${Url}/admin/exams`, this.formData)
                    .then(res => {
                        ...
                        this.message.title = res.data.msg;
                    });
            },
